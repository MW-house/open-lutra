...
This repository is a **pilot / verification (PoC) repository maintained specifically for MW, Inc.** It exists to validate the recording workflow against MW's environment and requirements. General-purpose reusability as an OSS project is **not** a goal here: prefer solutions that fit MW's concrete use case over abstractions aimed at arbitrary users. (The upstream OpenLUTRA project is the place for general-purpose, OSS-oriented work.)
